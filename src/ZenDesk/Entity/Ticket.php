...
class Ticket extends AbstractEntity implements RestParametersAware, FilterProviderInterface
{
    ...
    {
        $available = array(
            self::STATUS_OPEN, self::STATUS_CLOSED, self::STATUS_HOLD, self::STATUS_NEW,
            self::STATUS_PENDING, self::STATUS_SOLVED
        );
        if (!in_array($status, $available)) {
            throw new LogicException(sprintf('status value must be one of the following value "%s"', implode(', ', $available)));
        }
        $this->status = $status;
    }
